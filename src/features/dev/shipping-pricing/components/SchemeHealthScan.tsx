...
export const SchemeHealthScan: React.FC<{ schemeId: number | null }> = ({ schemeId }) => {
  ...
  const report = useMemo(() => {
    ...
    return {
      ...
      note:
        "旧 bracket 覆盖/空洞/重叠扫描已退出；当前扫描仅基于 zones / destination_groups / surcharge_configs 做主线结构检查。",
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs text-slate-500">
            zones / destination_groups / surcharge_configs 结构诊断。
          </div>
        </div>
        <button
          type="button"
      ...
      {!report ? (
        ...
        <div className="mt-3 space-y-3">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm">
            ...
            <div className="mt-1 text-slate-700">
              surcharge_configs：<span className="font-mono">{report.surcharge_configs}</span> · total_issues：
