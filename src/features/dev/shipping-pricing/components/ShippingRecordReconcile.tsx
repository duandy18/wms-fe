...
import {
  ...
  type ShippingRecord,
} from "../../../shipment/api/shippingRecordsApi";
...
function tryExtractFromMeta(meta: Record<string, unknown> | null | undefined) {
  ...
  const dest =
    ...
    (quoteSnapshot
      ? pickObj(quoteSnapshot, ["dest", "address", "receiver"])
      : null);

  const province =
    pickStr(dest, ["province", "prov"]) ??
    pickStr(m, ["province", "receiver_province"]);
  const city =
    pickStr(dest, ["city"]) ?? pickStr(m, ["city", "receiver_city"]);
  const district =
    pickStr(dest, ["district"]) ??
    pickStr(m, ["district", "receiver_district"]);

  const flagsArr = (() => {
    const arr =
      (quoteSnapshot && (quoteSnapshot["flags"] as unknown)) ??
      (m["flags"] as unknown);
...
export const ShippingRecordReconcile: React.FC<Props> = (p) => {
  ...
  const found = useMemo(() => {
    ...
    const costReal =
      record.cost_real != null ? Number(record.cost_real) : null;
    const costEst =
      record.cost_estimated != null ? Number(record.cost_estimated) : null;
  ...
  const run = async () => {
    ...
    try {
      ...
      const picked =
        [...list].sort((a, b) => (b.id ?? 0) - (a.id ?? 0))[0] ?? null;
      setRecord(picked);
    } catch (e: unknown) {
  ...
  const applyToLab = () => {
    ...
    if (found.ex.schemeId != null) {
      p.setSchemeIdText(String(found.ex.schemeId));
    }
  };
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5">
      <div className="text-sm font-semibold text-slate-800">
        运单对照（Pilot Reconcile）
      </div>
      <div className="mt-1 text-xs text-slate-500">
        拉取 shipping_record → 回填 Lab → 与 cost_real/cost_estimated 做差异对照。
      </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-6">
        <div className="md:col-span-2">
          ...
          <input
            ...
            value={recordIdText}
            onChange={(e) => setRecordIdText(e.target.value)}
            placeholder="例如 123"
          />
        </div>
        ...
        <div className="mt-3 space-y-3">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
            <div className="font-mono">
              id={record.id} · ref={record.order_ref}
            </div>
            <div className="mt-1 font-mono">
              carrier={record.carrier_name ?? "-"}(
              {record.carrier_code ?? "-"}) · tracking=
              {record.tracking_no ?? "-"}
            </div>
            <div className="mt-1 font-mono">
              weight_kg={record.weight_kg ?? "—"} · cost_est=
              {record.cost_estimated ?? "—"} · cost_real=
              {record.cost_real ?? "—"}
            </div>
          </div>
          ...
          {diff ? (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
              ...
              <div className="mt-2 font-mono text-xs">
                cost={diff.baseline.toFixed(2)} · calc={diff.calc.toFixed(2)} ·
                delta={diff.delta.toFixed(2)}
              </div>
            </div>
          ) : (
            <div className="text-xs text-slate-500">
              提示：先用 Lab 跑出 calc（左侧），并且运单需有 cost_estimated 或
              cost_real，才能自动 diff。
            </div>
          )}
