// src/features/dev/orders/controller/actions/useDevOrdersActions.ts
...
import type { RmaFromOrderPayload, LinesPayload } from "./types";
import { buildLinesFromFacts, buildSingleLineFallback } from "./helpers";
...
export function useDevOrdersActions(args: {
  ...
  // ================================
  // 动作注入（useDevOrdersActions 不允许直接 import API）
  // ================================
  ...
  const handleAction = useCallback(
    async (t: "pick" | "ship") => {
      ...
      try {
        const lines = buildSingleLineFallback(orderFacts);
  ...
  const handleFullFlow = useCallback(async () => {
    ...
    try {
      const lines = buildLinesFromFacts(orderFacts);
