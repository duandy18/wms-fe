...
function buildLineUomOptions(
  ...
  const source = aggregateUoms && aggregateUoms.length > 0
...
const ReceivingPurchasePage: React.FC = () => {
  ...
  const selectedDetail = useMemo(() => {
    return selectedReceiptNo
      ? (m.detailByReceiptNo[selectedReceiptNo] ?? null)
      : null;
  ...
  async function handleResolveBarcodeAtEntry(
    ...
    if (!selectedDetail) {
      const msg = "请先选择采购收货单，再进行识别";
      setScanError(msg);
      throw new Error(msg);
    }

    setResolvingEntryKey(`${lineNo}-${_index}`);
    setScanError("");
    setScanSuccess("");
    ...
    for (const line of selectedDetail.lines) {
      ...
      for (const draft of drafts) {
        ...
        if (!Number.isFinite(qty) || qty <= 0) {
          setSubmitError(`任务行 ${line.line_no} 的本次数量非法`);
          return;
        }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-4">
        ...
        {selectedReceiptNo && m.detailLoadingByReceiptNo[selectedReceiptNo] ? (
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">
            正在展开采购收货单…
          </div>
        ) : null}

        {selectedReceiptNo && m.detailErrorByReceiptNo[selectedReceiptNo] ? (
          <div className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700">
          ...
          <>
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
                ...
                <div>
                  ...
                  <div className="text-sm text-slate-900">
                    {selectedDetail.source_doc_no_snapshot || "-"}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-slate-500">仓库</div>
                  <div className="text-sm text-slate-900">
                    {selectedDetail.warehouse_name_snapshot ||
                      `仓库 ${selectedDetail.warehouse_id}`}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-slate-500">供应商</div>
                  <div className="text-sm text-slate-900">
                    {selectedDetail.counterparty_name_snapshot || "-"}
                  </div>
                </div>
                <div>
            ...
            {lastSubmit ? (
              <div className="text-xs text-slate-500">
                最近提交：操作单 #{lastSubmit.id} / 操作时间{" "}
                {lastSubmit.operated_at}
              </div>
            ) : null}
