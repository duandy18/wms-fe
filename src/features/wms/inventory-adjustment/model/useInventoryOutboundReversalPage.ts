...
import {
  fetchOutboundReversalDetail,
  fetchOutboundReversalOptions,
  submitOutboundReversal,
} from "../api/outboundReversalApi";
import type {
...
import {
  buildOutboundReversalOptionLabel,
  parseOutboundReversalRangeDays,
  parseOutboundReversalSourceType,
} from "../contracts/outboundReversal";
...
export function useInventoryOutboundReversalPage() {
  const [searchParams, setSearchParams] = useSearchParams();

  const initialDays = parseOutboundReversalRangeDays(searchParams.get("days"));
  const initialSourceType = parseOutboundReversalSourceType(searchParams.get("source_type"));
  const queryEventId = Number(searchParams.get("event_id") ?? "");

  const [rangeDays, setRangeDays] = useState<OutboundReversalRangeDays>(initialDays);
  const [sourceType, setSourceType] = useState<OutboundReversalSourceType | null>(
    initialSourceType,
  );

  const [options, setOptions] = useState<OutboundReversalOptionOut[]>([]);
  ...
  const [detail, setDetail] = useState<OutboundReversalDetailOut | null>(null);
  ...
  const [submitResult, setSubmitResult] = useState<OutboundReversalOut | null>(null);

  const syncSearch = useCallback(
    (
      nextDays: OutboundReversalRangeDays,
      nextSourceType: OutboundReversalSourceType | null,
  ...
  const loadOptions = useCallback(async () => {
    ...
    setOptionsError("");
    try {
      const data = await fetchOutboundReversalOptions({
        days: rangeDays,
        limit: 50,
      ...
      setOptions([]);
      setOptionsError(getErrorMessage(error, "加载出库冲回候选事件失败"));
    } finally {
  ...
  const loadDetail = useCallback(async (eventId: number) => {
    ...
    setDetailError("");
    try {
      const data = await fetchOutboundReversalDetail(eventId);
      setDetail(data);
    } catch (error) {
      setDetail(null);
      setDetailError(getErrorMessage(error, "加载出库事件详情失败"));
    } finally {
  ...
  const optionLabel = useCallback((option: OutboundReversalOptionOut): string => {
    return buildOutboundReversalOptionLabel(option);
  }, []);
  ...
  const selectRangeDays = useCallback((value: string) => {
    const nextDays = parseOutboundReversalRangeDays(value);
  ...
  const selectSourceType = useCallback((value: string) => {
    const nextSourceType = parseOutboundReversalSourceType(value);
  ...
  const submitCurrent = useCallback(async () => {
    ...
    setSubmitSuccess("");
    try {
      const out = await submitOutboundReversal(selectedEventId, {
      ...
      setSubmitError(getErrorMessage(error, "执行出库冲回失败"));
    } finally {
