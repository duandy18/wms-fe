...
// - 打开后自动 window.print()
// - 底部生成条码：PICKTASK:<task_id>（用于最后扫码确认出库）
...
export const PickTicketPrintModal: React.FC<Props> = ({
  ...
  const handoffCode = `PICKTASK:${task.id}`;
  ...
  const lines = useMemo(() => {
    ...
    return raw
      .map((ln) => {
        ...
        const qty = Number(ln.picked_qty ?? 0);
        ...
        return { itemId, name, sku, batch, qty };
      })
      .filter((x) => x.qty > 0);
      ...
      <div className="fixed inset-0 z-[61] flex items-center justify-center p-4">
        ...
        >
          <div className="flex items-start justify-between gap-2 border-b border-slate-200 pb-2">
            <div>
              <div className="text-sm font-semibold">拣货单</div>
                ...
                {lines.length === 0 ? (
                  <tr>
                    <td className="px-2 py-3 text-slate-500" colSpan={3}>
                      暂无 picked_qty&gt;0 的行（请先拣货/扫码）
                    </td>
                  </tr>
                ) : (
                  lines.map((ln, idx) => (
                    <tr key={`${ln.itemId}-${ln.batch}-${idx}`} className="border-t border-slate-200">
                      <td className="px-2 py-2">
                        <div className="font-medium text-slate-900">{ln.name}</div>
                        {ln.sku ? (
                          <div className="font-mono text-[10px] text-slate-500">{ln.sku}</div>
                        ) : null}
                      </td>
                      <td className="px-2 py-2 font-mono">{ln.batch || "-"}</td>
                      <td className="px-2 py-2 text-right font-semibold">{ln.qty}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          <div className="mt-4 border-t border-dashed border-slate-200 pt-3">
            <div className="text-[10px] text-slate-500">出库确认码（扫码后提交出库）</div>
