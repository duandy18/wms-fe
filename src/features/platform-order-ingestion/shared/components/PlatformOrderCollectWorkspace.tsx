...
import {
  mockAuthorizePlatformStore,
  mockClearPlatformOrders,
  mockIngestPlatformOrders,
} from "../api/mockApi";
import {
  createPlatformOrderPullJob,
  fetchPlatformOrderPullJobDetail,
  fetchPlatformOrderPullJobs,
  runPlatformOrderPullJobOnce,
  runPlatformOrderPullJobPages,
} from "../api/pullJobsApi";
import { fetchPlatformOrderIngestionStoreStatus } from "../api/statusApi";
import {
  fetchPlatformOrderIngestionStores,
  type PlatformOrderIngestionStoreOption,
} from "../api/storesApi";
import type { MockScenario, PlatformCode } from "../contracts/common";
import type {
  PullJob,
  PullJobRun,
  PullJobRunLog,
...
export type PlatformOrderCollectWorkspaceProps<TAppConfig> = {
  platform: PlatformCode;
  platformLabel: string;
  nativeOrdersPath: string;
  fetchAppConfig: () => Promise<TAppConfig>;
  renderAppConfigFields: (appConfig: TAppConfig | null) => AppConfigField[];
...
function FieldView(props: AppConfigField) {
  return (
    <div>
...
export function PlatformOrderCollectWorkspace<TAppConfig>(
  props: PlatformOrderCollectWorkspaceProps<TAppConfig>,
) {
  const [stores, setStores] = useState<PlatformOrderIngestionStoreOption[]>([]);
  const [storeId, setStoreId] = useState<number | null>(null);
  const [appConfig, setAppConfig] = useState<TAppConfig | null>(null);
  const [status, setStatus] =
    useState<PlatformOrderIngestionStoreStatus | null>(null);
  ...
  const [orderStatus, setOrderStatus] = useState(props.defaultOrderStatus ?? "1");
  const [pageSize, setPageSize] = useState(props.defaultPageSize ?? "50");
  ...
  async function refreshJobs(nextStoreId: number | null = storeId) {
    ...
    setJobs(data.rows);
    setSelectedJobId((current) => current ?? data.rows[0]?.id ?? null);
  }
  ...
  async function refreshSelectedJobDetail(
    nextJobId: number | null = selectedJobId,
  ) {
    if (!nextJobId) {
      setRuns([]);
      setLogs([]);
  ...
  async function loadInitial() {
    ...
    try {
      const [storeRows, config] = await Promise.all([
        fetchPlatformOrderIngestionStores(props.platform),
      ...
      await Promise.all([
        refreshJobs(firstStoreId),
        refreshStatus(firstStoreId),
      ]);
    } catch (err) {
      console.error(`load ${props.platform} collect page failed`, err);
      setError(
        err instanceof Error
          ? err.message
          : `加载${props.platformLabel}订单采集页失败`,
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className={poiUi.page}>
      <section className={poiUi.hero}>
        <div className={poiUi.pill}>{props.platformLabel}</div>
        <h1 className={poiUi.heroTitle}>{props.platformLabel}订单采集</h1>
        <p className={poiUi.heroDesc}>
          本页负责{props.platformLabel}平台订单采集链路：系统配置、店铺授权、连接状态、模拟联调、
          拉单任务和任务日志。订单拉取后只落到{props.platformLabel}原生订单表，不创建内部订单。
        </p>
        <div className="mt-4 flex flex-wrap gap-2">
          <Link to={props.nativeOrdersPath} className={poiUi.secondaryLink}>
            查看{props.platformLabel}原生订单台账
          </Link>
      ...
      <section className={poiUi.card}>
        ...
        <p className={poiUi.cardDesc}>
          选择一个{props.platformLabel}店铺后，页面会读取该店铺的采集状态、拉单任务和 mock 操作结果。
        </p>
        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
          <label>
            <div className={poiUi.label}>{props.platformLabel}店铺</div>
            <select
              ...
              onChange={(event) => {
                ...
                void Promise.all([
                  refreshStatus(nextStoreId),
                  refreshJobs(nextStoreId),
                ]).catch((err) =>
                  setError(err instanceof Error ? err.message : "切换店铺失败"),
                );
              }}
            >
      ...
      <section className={poiUi.grid2}>
        <section className={poiUi.card}>
          <h2 className={poiUi.cardTitle}>{props.platformLabel}系统配置</h2>
          <p className={poiUi.cardDesc}>
            读取当前启用中的{props.platformLabel}系统配置。
          </p>
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            {props.renderAppConfigFields(appConfig).map((field) => (
              <FieldView
                key={field.label}
                label={field.label}
                value={field.value}
        ...
        <section className={poiUi.card}>
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <FieldView
              label="凭证存在"
              value={status?.credential.present ?? null}
              badge
            />
            <FieldView
              label="凭证状态"
              value={status?.credential.credential_status ?? null}
              badge
            />
            <FieldView
              label="连接存在"
              value={status?.connection.present ?? null}
              badge
            />
            <FieldView
              label="连接状态"
              value={status?.connection.connection_status ?? null}
              badge
            />
            <FieldView
              label="需要重新授权"
              value={status?.connection.reauth_required ?? null}
              badge
            />
            <FieldView
              label="最近检测"
              value={status?.connection.last_pull_checked_at ?? null}
            />
          </div>
      ...
      <section className={poiUi.grid2}>
        ...
        <section className={poiUi.card}>
          ...
          <p className={poiUi.cardDesc}>
            创建并执行{props.platformLabel}拉单任务。真实拉单是否成功取决于授权、连接状态和平台接口。
          </p>
      ...
      <section className={poiUi.card}>
        ...
        <p className={poiUi.cardDesc}>
          这里展示当前店铺最近的{props.platformLabel}拉单任务、运行记录和日志。
        </p>
            ...
            <div className={poiUi.tableWrap}>
              <table className={poiUi.table}>
                ...
                <tbody>
                  ...
                  {!runs.length ? (
                    <tr>
                      <td className={poiUi.td} colSpan={6}>
                        暂无运行记录。
                      </td>
                    </tr>
                  ) : null}
        ...
        <div className="mt-4 max-h-80 overflow-auto rounded-2xl border border-slate-200">
          <table className={poiUi.table}>
            ...
            <tbody>
              ...
              {!logs.length ? (
                <tr>
                  <td className={poiUi.td} colSpan={5}>
                    暂无任务日志。
                  </td>
                </tr>
              ) : null}
