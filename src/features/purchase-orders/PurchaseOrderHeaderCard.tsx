...
export const PurchaseOrderHeaderCard: React.FC<PurchaseOrderHeaderCardProps> = ({
  ...
  const cardCls = "bg-white border border-slate-200 rounded-xl p-4 space-y-3";
  const titleCls = "text-sm font-semibold text-slate-800";
  const metaCls = "text-xs text-slate-500";
  const gridCls = "grid grid-cols-1 md:grid-cols-4 gap-x-8 gap-y-2 text-sm text-slate-700";
  const labelCls = "text-xs text-slate-500";
  const idCls = "font-mono text-[13px]";
