// src/features/shipment/cockpit/components/ShipmentInputPanel.tsx

import React, { useMemo } from "react";
import { UI } from "../ui";
import ShipmentHidScalePanel from "./ShipmentHidScalePanel";
import type {
  CandidateWarehouse,
  FulfillmentScanWarehouse,
  FulfillmentMissingLine,
...
function findMissing(
  scan: FulfillmentScanWarehouse | undefined | null,
): FulfillmentMissingLine[] {
  const m = scan?.missing;
  return Array.isArray(m) ? m : [];
}

export const ShipmentInputPanel: React.FC<Props> = ({
  orderRef,
  onOrderRefChange,
  ...
  const blocked =
    String(fulfillmentStatus || "").toUpperCase() === "FULFILLMENT_BLOCKED";
  ...
  return (
    <section className={UI.card}>
      ...
      <div className="mt-3 space-y-3">
        ...
        {warehouseReason ? (
          <div className="text-xs text-slate-500">提示：{warehouseReason}</div>
        ) : null}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead className="border-b border-slate-200 bg-white text-slate-600">
                  <tr>
                    <th className="w-12 px-3 py-2 text-left">序</th>
                    <th className="px-3 py-2 text-left">仓库</th>
                    <th className="w-20 px-3 py-2 text-left">优先级</th>
                    <th className="w-20 px-3 py-2 text-left">结果</th>
                    ...
                    const missingText =
                      miss.length === 0
                        ...
                        : miss
                            ...
                            .map(
                              (m) =>
                                `item=${m.item_id} need=${m.need} have=${m.available}`,
                            )
                            .join("；") +
        ...
        <ShipmentHidScalePanel
          onWeightLocked={(w) => onWeightChange(w.toFixed(3))}
        />

        <p className="text-sm text-slate-500">
          · 本流程不预设仓库、不兜底：先扫描候选仓可履约性，再由作业员选择起运仓。
          <br />
...
export default ShipmentInputPanel;
