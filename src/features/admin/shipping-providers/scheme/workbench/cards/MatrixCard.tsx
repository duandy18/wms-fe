// src/features/admin/shipping-providers/scheme/workbench/cards/MatrixCard.tsx
//
// 分拆说明：
      ...
      {!canEdit ? (
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full table-fixed border-collapse">
            <thead>
              <tr>
                <th className="w-[220px] min-w-[220px] border border-slate-200 bg-slate-50 px-3 py-2 text-left text-sm font-semibold text-slate-700">
                  地区范围
                </th>
                {columns.map((col) => (
                  <th
                    key={col.moduleRangeId}
                    className="w-[240px] min-w-[240px] border border-slate-200 bg-slate-50 px-3 py-2 text-left text-sm font-semibold text-slate-700"
                  >
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.groupId}>
                  ...
                  {row.cells.map((cell) => (
                    <td key={cell.key} className="border border-slate-200 px-3 py-3 align-top">
                      {renderCell(cell)}
                    </td>
                  ))}
                </tr>
          ...
          {rows.length === 0 || columns.length === 0 ? (
            <div className="mt-3 text-sm text-slate-600">当前模块的矩阵还没有可展示的行列。</div>
          ) : null}
        </div>
      )}
    </div>
