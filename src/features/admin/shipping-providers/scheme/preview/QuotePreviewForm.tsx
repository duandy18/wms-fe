...
export const QuotePreviewForm: React.FC<{
  ...
  return (
    <div className={UI.previewCard}>
      <div className={UI.sectionTitle}>输入条件</div>

      <div className={UI.previewGrid3}>
        <div className={UI.previewFieldCol}>
          <label className={UI.previewLabel}>省（标准值）</label>
          <select
            className={UI.previewSelect}
            value={province}
            disabled={disabled}
          ...
          <div className={UI.previewHint}>
            建议：省必须标准化（下拉），否则 Zone 命中会很容易失败。
          </div>
        </div>

        <div className={UI.previewFieldCol}>
          <label className={UI.previewLabel}>重点城市（仅用于少量附加费/特例）</label>
          <select
            className={UI.previewSelect}
        ...
        <div className={UI.previewFieldCol}>
          <label className={UI.previewLabel}>市（可手输，非重点城市时用）</label>
          <input
            className={UI.previewInput}
            value={city}
            disabled={disabled}
      ...
      <details className={UI.previewDetails}>
        <summary className={UI.previewDetailsSummary}>高级（可选）：区/县（目前多数规则不使用）</summary>

        <div className={UI.previewGrid3}>
          <div className={UI.previewFieldCol}>
            <label className={UI.previewLabel}>区/县</label>
            <input
              className={UI.previewInput}
              value={district}
              disabled={disabled}
      ...
      <div className={UI.previewGrid3}>
        <div className={UI.previewFieldCol}>
          <label className={UI.previewLabel}>实重（kg）*</label>
          <input
            className={UI.previewInputMono}
            value={realWeightKg}
            disabled={disabled}
        ...
        <div className={`${UI.previewFieldCol} md:col-span-2`}>
          <label className={UI.previewLabel}>flags（逗号分隔，可选）</label>
          <input
            className={UI.previewInputMono}
            value={flags}
            disabled={disabled}
      ...
      <div className={UI.previewGrid4}>
        <div className={UI.previewFieldCol}>
          <label className={UI.previewLabel}>长（cm，可选）</label>
          <input
            className={UI.previewInputMono}
            value={lengthCm}
            disabled={disabled}
        ...
        <div className={UI.previewFieldCol}>
          <label className={UI.previewLabel}>宽（cm，可选）</label>
          <input
            className={UI.previewInputMono}
            value={widthCm}
            disabled={disabled}
        ...
        <div className={UI.previewFieldCol}>
          <label className={UI.previewLabel}>高（cm，可选）</label>
          <input
            className={UI.previewInputMono}
            value={heightCm}
            disabled={disabled}
      ...
      {!showDimsWarning ? null : <div className={UI.previewDimsWarning}>体积重：需要同时填写 长/宽/高 三项才会参与计算（否则按未填写处理）。</div>}
    </div>
  );
};
