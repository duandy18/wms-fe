...
const ItemsFormSection: React.FC = () => {
  ...
  // SKU 片段：初始化时从 localStorage 读取，后续每次变更时写回
  ...
  // skuParts 变化时写回 localStorage，保持“记住上次选择”的能力
  useEffect(() => {
  ...
  const updateSkuPart = (field: keyof SkuParts, value: any) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaveError(
        e?.response?.data?.detail || e?.message || "创建商品失败",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
      {/* 左：新建商品表单 */}
      <div className="bg白 border border-slate-200 rounded-xl p-4 space-y-4">
              ...
              <div className="flex gap-1 flex-wrap mt-1">
                {["袋", "箱", "罐", "听", "个", "件"].map((u) => (
                  <button
                    key={u}
                    type="button"
                    className="px-2 py-0.5 text-[11px] border rounded-full hover:bg-slate-50"
      ...
      {/* 右：SKU 生成器（原样保留） */}
      <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-4 text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="flex flex-col gap-1">
            ...
            <select
              ...
              onChange={(e) =>
                updateSkuPart("category", e.target.value.toUpperCase())
              }
            >
            ...
            <input
              className="mt-1 border rounded-lg px-2 py-1 font-mono"
              value={skuParts.category}
              onChange={(e) =>
                updateSkuPart("category", e.target.value.toUpperCase())
              }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs text-slate-600">规格+口味编码(3+)</label>
            <input
              className="border rounded-lg px-2 py-1 font-mono"
              value={skuParts.specCode}
              onChange={(e) =>
                updateSkuPart("specCode", e.target.value.toUpperCase())
              }
        ...
        {/* 序号 + 按钮 */}
        <div className="grid grid-cols-[1.5fr_auto] gap-3 items-end">
          <div className="flex flex-col gap-1">
            <label className="text-xs text-slate-600">
              序号(3，自填 / 自动递增)
            </label>
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg border border-slate-300 text-xs font-medium hover:bg-slate-50"
          >
            使用当前编码生成 SKU
          </button>
        </div>
