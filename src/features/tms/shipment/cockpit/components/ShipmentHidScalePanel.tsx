// src/features/shipment/cockpit/components/ShipmentHidScalePanel.tsx
//
// 电子称状态面板（Shipment Cockpit 配套）
// - 使用 useHidScale Hook
// - 显示当前读数 / 锁定读数 / 设备信息
// - 把锁定值通过 onWeightLocked 回传给上层

import React, { useEffect } from "react";
import { useHidScale } from "../../hooks/useHidScale";

type Props = {
  onWeightLocked?: (weightKg: number) => void;
};

export const ShipmentHidScalePanel: React.FC<Props> = ({ onWeightLocked }) => {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-2 grid grid-cols-1 gap-4 text-xs text-slate-700 md:grid-cols-3">
      ...
      {rawText && (
        <div className="mt-3 rounded-md border border-slate-100 bg-slate-50 px-3 py-2 text-[10px] text-slate-500">
          ...
          <pre className="max-h-24 overflow-auto font-mono">{rawText}</pre>
        </div>
      )}
...
export default ShipmentHidScalePanel;
