// src/features/shipping-assist/billing/components/BillingItemsTable.tsx
          ...
          <table className="min-w-full border-collapse text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-3 py-2 text-left text-slate-600">长宽高</th>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-slate-100">
                  ...
                  <td className="px-3 py-2 text-xs">{row.sender_name ?? "-"}</td>
                  <td className="px-3 py-2 text-xs">{row.size_text ?? "-"}</td>
                  <td className="px-3 py-2 text-right font-mono text-xs">
                    {formatNumber(row.billing_weight_kg)}
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-xs">
                    {formatMoney(row.freight_amount)}
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-xs">
                    {formatMoney(row.surcharge_amount)}
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-xs">
