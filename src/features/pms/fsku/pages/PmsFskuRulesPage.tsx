...
import type { PmsFskuListItem, PmsFskuStatus } from "../types";
...
export default function PmsFskuRulesPage() {
  ...
  async function publishSelected(): Promise<void> {
    if (!selectedFsku || selectedFsku.status !== "draft") return;

    setSaving(true);
    setBanner(null);

    try {
      await publishPmsFsku(selectedFsku.id);
      setBanner({ kind: "success", text: `FSKU 已发布：#${selectedFsku.id}` });
      await loadFskus();
    } catch (err) {
    ...
    <div className="space-y-6 p-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          >
            {selectedFsku?.status === "draft" ? "替换草稿表达式" : "创建草稿"}
          </button>
          ...
          <button
            ...
            disabled={saving || selectedFsku?.status !== "draft"}
          >
            发布 FSKU
          </button>
        </div>
          ...
          <table className="min-w-[1100px] w-full text-left text-sm">
            <thead className="bg-slate-50 text-xs text-slate-500">
              <tr>
                ...
                <th className="px-3 py-2">SKU 组合</th>
                <th className="px-3 py-2">操作</th>
              </tr>
            </thead>
            <tbody>
              {fskuRows.map((row) => (
                <tr
                  key={row.id}
                  className={selectedFskuId === row.id ? "border-t border-slate-100 bg-sky-50/60" : "border-t border-slate-100"}
                >
                  <td className="px-3 py-2 font-mono text-xs">{row.code}</td>
                  <td className="px-3 py-2">{row.name}</td>
                  <td className="px-3 py-2">
                    <span className={`inline-flex rounded-full border px-2 py-0.5 text-xs font-medium ${statusCls(row.status)}`}>
                      {statusLabel(row.status)}
                    </span>
                  </td>
                  <td className="px-3 py-2 font-mono text-xs">{row.fsku_expr}</td>
                  <td className="px-3 py-2 text-xs">
                    {splitSummary(row.components_summary_name || row.components_summary).map((line) => (
