// src/features/receive-tasks/components/ReceiveTaskInfoCard.tsx

import React from "react";
import type { ReceiveTask } from "../api";
import { formatTs } from "../utils";

export type ReceiveVarianceSummary = {
  totalExpected: number;
  totalScanned: number;
  totalVariance: number;
};

export const ReceiveTaskInfoCard: React.FC<{
  task: ReceiveTask;
  varianceSummary: ReceiveVarianceSummary;
  ...
  return (
    <section className="space-y-2 rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-700">
      ...
      <div className="grid grid-cols-1 gap-x-8 gap-y-2 md:grid-cols-3">
        <div>
          <div className="text-[11px] text-slate-500">收货任务 ID</div>
        ...
        <div>
          <div className="text-[11px] text-slate-500">应收 vs 实收</div>
          <div>
            应收：{varianceSummary.totalExpected}，实收：{varianceSummary.totalScanned}
            ，差异：
            <span
              className={
