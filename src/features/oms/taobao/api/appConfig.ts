...
export async function fetchTaobaoAuthorizeStart(
  storeId: number,
): Promise<TaobaoAuthorizeStartResult> {
  const resp = await apiGet<{ ok: boolean; data: TaobaoAuthorizeStartResult }>(
    `/oms/taobao/oauth/start?store_id=${storeId}`,
  );
  return assertOk(resp, "GET /oms/taobao/oauth/start");
}
