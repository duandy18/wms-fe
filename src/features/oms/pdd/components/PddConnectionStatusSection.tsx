...
export default function PddConnectionStatusSection() {
  const [storeIdInput, setStoreIdInput] = useState("918");
  const [connection, setConnection] = useState<PddConnectionStatus | null>(null);
  const [authorizeResult, setAuthorizeResult] =
    useState<PddAuthorizeStartResult | null>(null);
  ...
  const loadConnection = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchPddConnection(storeId);
      setConnection(data);
    } catch (err) {
      console.error("load pdd connection failed", err);
      setConnection(null);
      setError("加载拼多多连接状态失败。");
    } finally {
      setLoading(false);
    }
  }, [storeIdInput]);
  ...
  async function handleAuthorizeStart() {
    ...
    setOk(null);
    try {
      const data = await fetchPddAuthorizeStart(storeId);
      setAuthorizeResult(data);
      setOk("拼多多授权链接已生成。");
    } catch (err) {
      console.error("generate pdd authorize url failed", err);
      setAuthorizeResult(null);
      setError("生成拼多多授权链接失败。");
    } finally {
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="mt-1 text-sm font-medium text-slate-900">
            {credential?.granted_identity_display ||
              credential?.granted_identity_value ||
              "—"}
          </div>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          <div className="text-xs text-slate-500">credential_type</div>
          <div className="mt-1 text-sm font-medium text-slate-900">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          <div className="text-xs text-slate-500">credential_expires_at</div>
          <div className="mt-1 text-sm font-medium text-slate-900">
            {formatTime(credential?.expires_at ?? null)}
          </div>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          <div className="text-xs text-slate-500">credential_updated_at</div>
          <div className="mt-1 text-sm font-medium text-slate-900">
            {formatTime(credential?.updated_at ?? null)}
          </div>
        </div>
      </div>
