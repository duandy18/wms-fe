// src/features/oms/fsku/api_merchant_code_bindings.ts
...
function unwrapMerchantCodeBindingsList(raw: unknown): MerchantCodeBindingsList {
  if (!isObject(raw)) throw new Error(`合同不匹配：GET /oms/merchant-code-bindings 返回 ${kindOf(raw)}，期望对象`);
  const ok = (raw as { ok?: unknown }).ok;
  if (ok !== true) throw new Error(`合同不匹配：GET /oms/merchant-code-bindings.ok 非 true`);

  const data = (raw as { data?: unknown }).data;
  if (!isObject(data)) throw new Error(`合同不匹配：GET /oms/merchant-code-bindings.data 为 ${kindOf(data)}，期望对象`);

  const itemsU = (data as { items?: unknown }).items;
  if (!Array.isArray(itemsU)) throw new Error(`合同不匹配：GET /oms/merchant-code-bindings.data.items 为 ${kindOf(itemsU)}，期望数组`);
  ...
  if (total == null) throw new Error(`合同不匹配：GET /oms/merchant-code-bindings.data.total 非法`);
  if (limit == null || limit < 1) throw new Error(`合同不匹配：GET /oms/merchant-code-bindings.data.limit 非法`);
  if (offset == null) throw new Error(`合同不匹配：GET /oms/merchant-code-bindings.data.offset 非法`);
...
export async function apiListMerchantCodeBindings(args: {
  ...
  const raw = await apiFetchJson<unknown>(`/oms/merchant-code-bindings${q}`, { method: "GET" });
...
export async function apiBindMerchantCode(args: {
  ...
  const raw = await apiFetchJson<unknown>("/oms/merchant-code-bindings/bind", {
    method: "POST",
    body: JSON.stringify({
      platform: args.platform,
      shop_id: args.shop_id,
  ...
  if (!isObject(raw)) throw new Error(`合同不匹配：POST /oms/merchant-code-bindings/bind 返回 ${kindOf(raw)}，期望对象`);
  const ok = (raw as { ok?: unknown }).ok;
  if (ok !== true) throw new Error(`合同不匹配：POST /oms/merchant-code-bindings/bind.ok 非 true`);
  const data = (raw as { data?: unknown }).data;
...
export async function apiUnbindMerchantCodeBinding(args: {
  ...
  if (!isObject(raw)) throw new Error(`合同不匹配：POST /oms/merchant-code-bindings/close 返回 ${kindOf(raw)}，期望对象`);
  const ok = (raw as { ok?: unknown }).ok;
  if (ok !== true) throw new Error(`合同不匹配：POST /oms/merchant-code-bindings/close.ok 非 true`);
  const data = (raw as { data?: unknown }).data;
  return parseMerchantCodeBindingRow(data, 0);
}
